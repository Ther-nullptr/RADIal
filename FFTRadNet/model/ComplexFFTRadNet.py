...
from .complex_layers import ComplexConv2d, ComplexBatchNorm2d, ComplexReLU, ComplexConvTranspose2d, complex_conv3x3, ComplexBasicBlock
...
class ComplexDetection_Header(nn.Module):
    ...
    def __init__(self, use_bn=True, reg_layer=2, input_angle_size=0):
        ...
        if(self.input_angle_size==224):
            self.conv1 = complex_conv3x3(256, 144, bias=bias)
            self.bn1 = ComplexBatchNorm2d(144)
            self.conv2 = complex_conv3x3(144, 96, bias=bias)
            self.bn2 = ComplexBatchNorm2d(96)
        elif(self.input_angle_size==448):
            self.conv1 = ComplexConv2d(256, 144, kernel_size=3, stride=(1,2), padding=1, bias=bias)
            self.bn1 = ComplexBatchNorm2d(144)
            self.conv2 = complex_conv3x3(144, 96, bias=bias)
            self.bn2 = ComplexBatchNorm2d(96)
            ...
            self.conv1 = ComplexConv2d(256, 144, kernel_size=3, stride=(1,2), padding=1, bias=bias)
            self.bn1 = ComplexBatchNorm2d(144)
            self.conv2 = ComplexConv2d(144, 96, kernel_size=3, stride=(1,2), padding=1, bias=bias)
            self.bn2 = ComplexBatchNorm2d(96)
        elif(self.input_angle_size==1024):
            self.conv1 = ComplexConv2d(256, 144, kernel_size=3, stride=(1,4), padding=1, bias=bias)
            self.bn1 = ComplexBatchNorm2d(144)
            self.conv2 = ComplexConv2d(144, 96, kernel_size=3, stride=(1,2), padding=1, bias=bias)
            self.bn2 = ComplexBatchNorm2d(96)
        else:
        ...
        self.conv3 = complex_conv3x3(96, 96, bias=bias)
        self.bn3 = ComplexBatchNorm2d(96)
        self.conv4 = complex_conv3x3(96, 96, bias=bias)
...
class ComplexBottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, downsample=None, expansion=4):
        ...
        self.conv2 = ComplexConv2d(planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn2 = ComplexBatchNorm2d(planes)
        self.conv3 = ComplexConv2d(planes, expansion*planes, kernel_size=1, bias=False)
...
class ComplexMIMO_PreEncoder(nn.Module):
    ...
    def __init__(self, in_layer, out_layer, kernel_size=(1,12), dilation=(1,16), use_bn=False):
        ...
        self.conv = ComplexConv2d(in_layer, out_layer, kernel_size,
...
class ComplexFPN_BackBone(nn.Module):
    ...
    def __init__(self, num_block, channels, block_expansion, mimo_layer, use_bn=True):
        ...
        self.conv = complex_conv3x3(self.in_planes, self.in_planes)
    ...
    def _make_layer(self, block, planes, num_blocks):
        if self.use_bn:
            downsample = nn.Sequential(
                ComplexConv2d(self.in_planes, planes * self.block_expansion,
                             kernel_size=1, stride=2, bias=False),
                ComplexBatchNorm2d(planes * self.block_expansion)
            )
        else:
            downsample = ComplexConv2d(self.in_planes, planes * self.block_expansion,
...
class ComplexRangeAngle_Decoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.deconv4 = ComplexConvTranspose2d(16, 16, kernel_size=3, stride=(2,1), padding=1, output_padding=(1,0))

        self.conv_block4 = ComplexBasicBlock(48, 128)  # 16 + 32 = 48
        self.deconv3 = ComplexConvTranspose2d(128, 128, kernel_size=3, stride=(2,1), padding=1, output_padding=(1,0))
        self.conv_block3 = ComplexBasicBlock(192, 256)  # 128 + 64 = 192

        # Complex channel mapping layers
        self.L3 = ComplexConv2d(192, 32, kernel_size=1, stride=1, padding=0)
        self.L2 = ComplexConv2d(160, 64, kernel_size=1, stride=1, padding=0)
