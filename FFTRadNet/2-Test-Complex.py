...
import torch
import numpy as np
from model.ComplexFFTRadNet import ComplexFFTRadNet
from dataset.complexdataset import ComplexRADIal
...
def main(config, checkpoint_filename,difficult):
    ...
    dataset = ComplexRADIal(root_dir = config['dataset']['root_dir'],
                        statistics= config['dataset']['statistics'],
                        encoder=enc.encode,
                        difficult=difficult)
    ...
    # Create the model
    net = ComplexFFTRadNet(blocks = config['model']['backbone_block'],
    ...
    for data in dataset:
        ...
        cv2.imshow('ComplexFFTRadNet',hmi)
...
if __name__=='__main__':
    # PARSE THE ARGS
    parser = argparse.ArgumentParser(description='ComplexFFTRadNet test')
